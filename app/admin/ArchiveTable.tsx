...
export type ArchiveWork = {
  ...
  category: string | null; language: string | null; views: number | null;
  downloads: number | null; pdf_file: string | null; external_url: string | null; created_at: string;
};
...
export default function ArchiveTable({ works, onWorksChange }: { works: ArchiveWork[]; onWorksChange?: (works: ArchiveWork[]) => void }) {
  ...
  const languages = uniqueValues(items.map((work) => work.language));
  const categories = uniqueValues(items.map((work) => work.category));
  ...
  async function deleteWork(work: ArchiveWork) {
    ...
    try {
      ...
      setItems(nextItems); onWorksChange?.(nextItems);
  ...
  return <>
    <div className="border-b border-gray-100 p-4 sm:p-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><div className="flex flex-col gap-2 sm:flex-row">
          ...
          <select value={language} onChange={(event) => setLanguage(event.target.value)} aria-label="Filter by language" className="rounded-lg bg-white px-3 py-2.5 text-sm text-gray-700 outline-none ring-1 ring-gray-200"><option value="">All languages</option>{languages.map((option) => <option key={option}>{option}</option>)}</select>
          <select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category" className="rounded-lg bg-white px-3 py-2.5 text-sm text-gray-700 outline-none ring-1 ring-gray-200"><option value="">All categories</option>{categories.map((option) => <option key={option}>{option}</option>)}</select>
          {hasFilters && <button type="button" onClick={() => { setSearch(""); setLanguage(""); setCategory(""); }} className="rounded-lg px-3 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50">Clear</button>}
        </div><AddWorkButton /></div>
      </div>
    </div>
    {error && <div role="alert" className="mx-4 mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
...
function uniqueValues(values: (string | null)[]) { return [...new Set(values.filter((value): value is string => Boolean(value)))].sort(); }
...
function SourceBadge({ work }: { work: ArchiveWork }) { if (work.pdf_file) return <span className="inline-flex rounded-full bg-green-50 px-2.5 py-1 text-xs font-medium text-green-700">PDF</span>; if (work.external_url) return <span className="inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">External</span>; return <span className="text-gray-400">—</span>; }
